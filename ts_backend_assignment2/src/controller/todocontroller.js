...
const { createtodoTable } = require('../models/todoModel');
const { todoIdSchema, todoSchema } = require('../schema/todoschema');
...
const TodoController = {
  ...
  getTodoById: async (req, res) => {
    try {
      const { error } = todoIdSchema.validate({ id: req.params.id });
      if (error) {
  ...
  updateTodo: async (req, res) => {
    try {
      const { error: idError } = todoIdSchema.validate({ id: req.params.id });
      if (idError) {
        return res.status(400).json({ error: idError.message });
      }

      const { error: todoError } = todoSchema.validate(req.body);
      if (todoError) {
        return res.status(400).json({ error: todoError.message });
      }
